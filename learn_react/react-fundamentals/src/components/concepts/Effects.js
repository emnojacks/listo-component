...
//even identical components run in isolation - their use states dont simultaneously change
...
const SampleEffect = () => {
    ...
    useEffect(() => {
        if (timerRunning) {
            window.alert("The timer is running down the street");
        }
    });
...
<button onClick ={buttonHandler}>Click me to start an effect in the console</button>
    ...
    return (
        <div style={{ border: '1px dashed black' }}>
            <h2> This components shows an effect</h2>
            <button>Click to start an effect in the console</button>
        </div>
    );
};